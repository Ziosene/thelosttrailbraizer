...
function LogPanel({ entries, onClose }: { entries: LogEntry[]; onClose: () => void }) {
  ...
  // Scroll to bottom when new entries arrive (log is newest-first, so scroll to top)
  useEffect(() => {
...
export function GamePage({ gameCode }: GamePageProps) {
  ...
  const { gameState, hand, myAddons, pendingChoice, reactionWindow, log, connect, disconnect, send, clearPendingChoice } = useGameStore()
