...
│       └── engine_cards/         ✅ effetti carte azione (60/300 implementate)
│           ├── __init__.py       ✅ dispatcher apply_action_card_effect
│           ├── helpers.py        ✅ get_target()
│           ├── economica.py      ✅ carte 1–8, 41–48
│           ├── offensiva.py      ✅ carte 9–18, 49–54
│           ├── difensiva.py      ✅ carte 19–25, 55–58
│           ├── manipolazione.py  ✅ carte 26–30, 59–60 (dice manipulation)
...
- **engine_cards/** (package): effetti carte azione. Entry-point: `apply_action_card_effect(card, player, game, db, *, target_player_id)`. Un modulo per categoria: `economica` (1–8, 41–48), `offensiva` (9–18, 49–54), `difensiva` (19–25, 55–58), `manipolazione` (26–30, 59–60), `utilita` (31–37), `interferenza` (38–40). Per aggiungere carte: apri il modulo della categoria, aggiungi una funzione e la chiave al dict esposto. `__init__.py` unisce tutti i dict e fa il dispatch per numero carta.
...
- [x] **Effetti carte azione — carte 1–60 implementate** — Package `engine_cards/` con 6 moduli: `economica` (1–8, 41–48), `offensiva` (9–18, 49–54), `difensiva` (19–25, 55–58), `manipolazione` (26–30, 59–60), `utilita` (31–37), `interferenza` (38–40). `combat_state` keys aggiunte batch 1: `boss_ability_disabled_until_round` (10, 21), `double_damage_until_round` (12), `boss_threshold_reduction` (13), `boss_threshold_increase_until_round` (15), `addons_blocked_next_turn` (18), `disaster_recovery_ready` (23), `next_roll_forced` (26), `critical_system_until_round` (28), `chaos_mode_next_roll` (29), `force_field_until_round` (30), `free_trial_addon_player_addon_ids` (37), `consulting_hours_until_round` + `consulting_hours_threshold_reduction` (38). Keys batch 2 (41–60): `fought_this_turn` (42, set in start_combat, cleared in draw_card), `drip_program_remaining` (43), `object_store_licenze` (44), `next_addon_price_fixed` (47), `next_addon_price_discount` (48), `ampscript_reflected_until_round` (53), `try_scope_until_round` (55), `on_error_continue_ready` (56), `api_proxy_active` (57, TODO pieno), `entitlement_process_until_round` (58), `dynamic_content_reroll` (59), `einstein_sto_next_roll_bonus` (60). Hooks combat.py: einstein_sto (pre-roll) → dynamic_content_reroll (post-LuckyRoll, on miss) → try_scope+entitlement+ampscript (miss branch) → on_error_continue (pre-death). Hooks turn.py draw_card: object_store return → drip delivery → fought_this_turn clear. Hooks turn.py buy_addon: next_addon_price_fixed / next_addon_price_discount. start_combat: preserva cross-turn flags + setta fought_this_turn.
